        ...
        <div class="col-md-12 content-wrapper" id="main-content">
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h4 class="text-center">Изменить страницу книги {{$book['title_' . Lang::getLocale()]}}</h4>
                </div>
                ...
                <div class="panel-body">
                    ...
                    <form class="" action="{{ route('book.page.update', [$book, $page]) }}" method="POST" id="create-document-form">

                        <input type="hidden" name="_method" value="PUT">
                        <input type="hidden" name="_token" value="{{ csrf_token() }}"> 

                        <div class="form-group with-radio">
                            <label><input type="radio" name="is_link" value="" @if(!$page->is_link) checked @endif>Не создавать ссылку в оглавлении</label>
                            <label><input type="radio" name="is_link" value="true" @if($page->is_link) checked @endif>Создать ссылку в оглавлении</label>
                        </div>
                        
                        <div class="form-group {{ $errors->has('title_ru') ? 'has-error' : '' }}">
                            <label for="title_ru">{{trans('admin.title')}} (рус.)</label>
                            <input type="text" class="form-control" id="title_ru" name="title_ru" required value="{{ $page->title_ru }}">
                        </div>
                        ...
                        <div class="form-group {{ $errors->has('position') ? 'has-error' : '' }}">
                            ...
                            <select name="position">
                                ...
                                @foreach( $pages as $index => $p )
                                    <option value="{{ $index + 1 }}" @if($index+1 == $page->position) selected @endif>{{ $index + 1 }}</option>
                                @endforeach
                            ...
                            <div>
                                ...
                                <div class="tab-content" id="content-wrapper">
                                    <div role="tabpanel" class="tab-pane active" id="content_ru-wrapper">
                                        <textarea name="content_ru" class="form-control" id="content_ru">{{$page->content_ru}}</textarea>
                                    ...
                                    <div role="tabpanel" class="tab-pane" id="content_en-wrapper">
                                        <textarea name="content_en" class="form-control" id="content_en">{{$page->content_en}}</textarea>
                    ...
                    <a href="{{ route('book.page.index', $book) }}" class="help-href">Все сраницы</a>
                </div> 
            </div>
        </div>
